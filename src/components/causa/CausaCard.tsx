...
export const CausaCard = ({ causa }: { causa: Causas }) => {
  ...
  return (
    <Card.Root
      ...
      className="flex max-md:place-self-center w-3/4 md:w-full flex-col overflow-hidden rounded bg-white md:flex-row md:justify-center"
    >
      <Card.Image
        alt="Imagem da causa"
        imageUrl="/mato.jpg"
        className="w-[240px] max-md:h-[240px] max-md:w-full h-full"
      />
      <Card.Content className="flex flex-col md:w-3/5 md:justify-between md:gap-2 md:p-4 2xl:w-full">
        <div className="max-h-[100px]">
          <h1 className="text-center text-3xl max-md:mb-2 md:text-xl font-bold text-wrap">
            {causa.nome}
          </h1>
          <p className="text-wrap line-clamp-2">{causa.descricao}</p>
        </div>
        <div className="flex flex-col gap-2">
          <div className="h-2 w-full rounded bg-gray-200">
            <div
              className="bg-accent h-full rounded max-w-full"
        ...
        <Card.Actions className="flex w-4/5 justify-center self-center gap-2 border-none p-0">
          <Button className="" onClick={() => router.push(`/causas/${causa.idCausa}`)}>
            Ver Detalhes
          </Button>
        </Card.Actions>
      </Card.Content>
